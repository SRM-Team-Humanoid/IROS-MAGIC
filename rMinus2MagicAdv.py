...
#----------------------------------------------------------------------------------------------------------------
darwin = {1: 90, 2: -90, 3: 67.5, 4: -70.5, 7: 45, 8: -45, 9: 'i', 10: 'i', 13: 'i', 14: 'i', 17: 'i', 18: 'i'}
roll = {1:75,2:-75,3:-28,4:28,5:60,6:-60,11:5,12:-5,15:-5,16:5}
tree = XmlTree('data.xml')
tree2 = XmlTree('Darwin.xml')
balance2 = MotionSet(tree2.parsexml("lift"), exclude=range(7,22))
...
w6 = MotionSet(tree.parsexml("37 "),speed=0.0001,offsets=[darwin])
shake = MotionSet(tree2.parsexml("last"), speed = 2.1)
...
if __name__ == '__main__':
	...
	head = Head(dxl)
	time.sleep(1)
	mixer.music.load('audio_files/Intro.mp3')
	rollpose.execute()
	time.sleep(3)
	#print state
	while len(done) < 5:
		...
		rollpose.execute()
		mixer.music.play()
		raw_input("Should I roll the die ?")
		rollDice()
		time.sleep(0.5)
		balance.execute()
		time.sleep(0.5)
		...
		if int(num) not in done and int(num) != 0:
			done.append(int(num))
			print "The number on upper face of the die is " + str(num)
			...
			mixer.music.load('audio_files/andha.mp3')
			mixer.music.play()
			time.sleep(2)
		else:
	...
	shake.execute()
	rollpose.execute()
	mixer.music.load('audio_files/Khatam.mp3')
	mixer.music.play()
	time.sleep(2)
